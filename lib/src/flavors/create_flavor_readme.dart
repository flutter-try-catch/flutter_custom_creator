...
/// Appends a comprehensive flavors section to the project's `README.md`
/// explaining how to use flavors, run/build with specific flavors,
/// access FlavorConfig, and add new flavors.
Future<void> createFlavorReadme({required String appName}) async {
  ...
  if (readmeFile.existsSync()) {
    existingContent = readmeFile.readAsStringSync();
  }
...
|----------|----------------------|----------------------------|----------------------------|
| dev      | $displayName Dev     | `com.example.$appName.dev`     | `com.example.$appName.dev`     |
| staging  | $displayName Staging | `com.example.$appName.staging` | `com.example.$appName.staging` |
| prod     | $displayName         | `com.example.$appName`         | `com.example.$appName`         |
  ...
  flavors:
    qa:
      app:
        name: "$displayName QA"
      android:
        applicationId: "com.example.$appName.qa"
      ios:
        bundleId: "com.example.$appName.qa"
```
...
''';
  ...
  readmeFile.writeAsStringSync(existingContent + flavorsSection);
  print('Updated README.md with flavors documentation');
}
